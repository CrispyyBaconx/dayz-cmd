...
- update `HIVE_ENABLED` in `core/CommunityOfflineClient.c`
- assemble offline launch arguments

This module should be mostly pure except for the file edit to `CommunityOfflineClient.c`.
...
5. If no managed DCOM copy exists, the browser screen offers install.
...
3. App extracts it under `offline/releases/<tag>/`, preserving the `Missions/...` subtree needed for launch.
4. App updates `offline/state.json`.
5. App refreshes the offline mission list.
...
5. App computes a sync plan from managed mission content to `DayZ/Missions/<mission>`.
6. If the target differs from the managed copy, app asks before overwriting.
7. App applies the sync.
8. App updates `HIVE_ENABLED`.
9. App assembles launch args and launches DayZ.
...
2. App skips managed sync.
3. User configures mods and spawn toggle.
4. App updates `HIVE_ENABLED` in place.
5. App launches with the offline args.
